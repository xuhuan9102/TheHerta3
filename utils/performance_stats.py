...
class ExportLogCollector:
    ...
    def generate_log_report(self) -> str:
        ...
        report = []
        report.append("=" * 100)
        report.append("导出流程日志")
        report.append("=" * 100)
        
        if self._start_time:
            report.append(f"导出开始时间: {self._start_time.strftime('%Y-%m-%d %H:%M:%S')}")
            report.append(f"导出结束时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        
        report.append(f"日志条目数: {len(self.logs)}")
        report.append("")
        report.append("-" * 100)
        report.append("详细日志")
        report.append("-" * 100)
    ...
    def save_to_text_editor(self, text_name: str = "导出流程日志"):
        """将日志保存到Blender内置文本编辑器"""
        try:
            import bpy
            ...
            print(f"导出流程日志已保存到文本编辑器: {text_name}")
            return True
        except Exception as e:
            print(f"保存导出流程日志到文本编辑器失败: {e}")
            return False
...
_global_log_collector = ExportLogCollector()
...
def start_log_collecting():
    """开始收集日志"""
    _global_log_collector.start_collecting()


def stop_log_collecting():
    """停止收集日志"""
    _global_log_collector.stop_collecting()


def save_export_log_to_editor(text_name: str = "导出流程日志"):
    """保存导出流程日志到文本编辑器"""
    return _global_log_collector.save_to_text_editor(text_name)


def clear_export_log():
    """清空导出流程日志"""
    _global_log_collector.clear()
