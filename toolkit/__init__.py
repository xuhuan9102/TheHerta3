...
def register():
    print("[TheHerta3] 注册工具集...")
    
    for op_class in vg_properties_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册属性: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册属性失败: {op_class.__name__} - {e}")
    ...
    for op_class in model_operators_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册失败: {op_class.__name__} - {e}")
    ...
    for op_class in bmtp_properties_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册BMTP属性: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册BMTP属性失败: {op_class.__name__} - {e}")
    ...
    for op_class in bmtp_bone_tools_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册BMTP骨骼工具: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册BMTP骨骼工具失败: {op_class.__name__} - {e}")
    
    for op_class in bmtp_weight_tools_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册BMTP权重工具: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册BMTP权重工具失败: {op_class.__name__} - {e}")
    
    for op_class in bmtp_clean_tools_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册BMTP清理工具: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册BMTP清理工具失败: {op_class.__name__} - {e}")
    
    for op_class in bmtp_material_tools_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册BMTP材质工具: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册BMTP材质工具失败: {op_class.__name__} - {e}")
    
    for op_class in bmtp_collection_linker_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册BMTP集合关联工具: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册BMTP集合关联工具失败: {op_class.__name__} - {e}")
    
    for op_class in bmtp_mesh_tools_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册BMTP网格工具: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册BMTP网格工具失败: {op_class.__name__} - {e}")
    
    for op_class in bmtp_modifier_tools_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册BMTP修改器工具: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册BMTP修改器工具失败: {op_class.__name__} - {e}")
    
    for op_class in bmtp_uv_tools_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册BMTP UV工具: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册BMTP UV工具失败: {op_class.__name__} - {e}")
    
    for op_class in tt_properties_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册TT属性: {op_class.__name__}")
        except Exception as e:
    ...
    for op_class in tt_dependency_check_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册TT依赖检查: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册TT依赖检查失败: {op_class.__name__} - {e}")
    
    for op_class in tt_dds_conversion_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册TT DDS转换: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册TT DDS转换失败: {op_class.__name__} - {e}")
    
    for op_class in tt_normal_map_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册TT法线贴图: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册TT法线贴图失败: {op_class.__name__} - {e}")
    
    for op_class in tt_color_bake_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册TT颜色烘焙: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册TT颜色烘焙失败: {op_class.__name__} - {e}")
    
    for op_class in tt_alpha_extract_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册TT Alpha提取: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册TT Alpha提取失败: {op_class.__name__} - {e}")
    
    for op_class in tt_lightmap_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册TT光照贴图: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册TT光照贴图失败: {op_class.__name__} - {e}")
    
    for op_class in tt_material_tools_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册TT材质工具: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册TT材质工具失败: {op_class.__name__} - {e}")
    
    for op_class in tt_material_preview_list[1:]:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册TT材质预览: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册TT材质预览失败: {op_class.__name__} - {e}")
    
    print("[TheHerta3] 注册动画处理工具...")
    for op_class in at_properties_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册AT属性: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册AT属性失败: {op_class.__name__} - {e}")
    ...
    for op_class in at_shape_key_control_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册AT形态键控制: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册AT形态键控制失败: {op_class.__name__} - {e}")
    
    for op_class in at_shape_key_operations_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册AT形态键操作: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册AT形态键操作失败: {op_class.__name__} - {e}")
    
    for op_class in at_shape_key_creation_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册AT形态键创建: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册AT形态键创建失败: {op_class.__name__} - {e}")
    
    for op_class in at_alembic_tools_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册AT Alembic工具: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册AT Alembic工具失败: {op_class.__name__} - {e}")
    
    for op_class in at_multi_frame_split_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册AT多帧拆分: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册AT多帧拆分失败: {op_class.__name__} - {e}")
    
    for op_class in at_batch_export_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册AT批量导出: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册AT批量导出失败: {op_class.__name__} - {e}")
    
    for op_class in at_shape_key_export_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册AT形态键导出: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册AT形态键导出失败: {op_class.__name__} - {e}")
    
    for op_class in at_animation_export_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册AT动画导出: {op_class.__name__}")
        except Exception as e:
            print(f"[TheHerta3]   注册AT动画导出失败: {op_class.__name__} - {e}")
    
    for op_class in at_buffer_merge_list:
        try:
            bpy.utils.register_class(op_class)
            print(f"[TheHerta3]   已注册AT缓冲合并: {op_class.__name__}")
        except Exception as e:
