...
class TT_OT_convert_to_dds(bpy.types.Operator):
    ...
    bl_label = "批量转换为 .dds (真实格式)"
    bl_description = "【真实转换】使用texconv.exe将输出目录的图片转换为指定DDS格式，并更新项目引用"
    bl_options = {'REGISTER', 'UNDO'}
    
    def _find_texconv(self, context):
        try:
            addon_dir = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
            local_path = os.path.join(addon_dir, 'Toolset', 'texconv.exe')
            if os.path.exists(local_path): return local_path
        except Exception: pass
        props = context.scene.texture_tools_props
        if props.texconv_path and os.path.exists(props.texconv_path): return props.texconv_path
        return shutil.which("texconv")
    
    def _get_dds_format_for_file(self, filename, props):
        import re
        ...
        type_identifier = name_no_ext.split('_')[0].lower() if '_' in name_no_ext else ""
        
        if type_identifier == 'diffusemap':
            return 'bc7_unorm_srgb'
        elif type_identifier == 'normalmap':
    ...
    def execute(self, context):
        ...
        output_dir_abs = bpy.path.abspath(props.output_dir)
        if not os.path.isdir(output_dir_abs): self.report({'ERROR'}, f"输出目录不存在: {output_dir_abs}"); return {'CANCELLED'}
        texconv_executable = self._find_texconv(context)
        if not texconv_executable: self.report({'ERROR'}, "未找到 texconv.exe。请将其放入插件目录的 'Toolset' 子文件夹，或手动指定路径。"); return {'CANCELLED'}
        ...
        for root, _, files in os.walk(output_dir_abs):
            for filename in files:
                ...
                new_path = os.path.join(root, f"{name_no_ext}.dds")
                
                dds_format = self._get_dds_format_for_file(filename, props)
                
                command = [texconv_executable, "-f", dds_format, "-o", root, "-y", old_path]
                if "_srgb" in dds_format: command.append("-srgb")
                
                try:
                    process = subprocess.run(command, capture_output=True, text=True, check=True, encoding='utf-8', errors='ignore')
                    if process.returncode == 0:
                        conversion_map[os.path.normpath(old_path)] = os.path.normpath(new_path)
                        converted_files_count += 1
                        if props.dds_delete_originals: os.remove(old_path)
                    else: self.report({'WARNING'}, f"转换文件 {filename} 失败: {process.stderr}")
                except Exception as e: self.report({'WARNING'}, f"处理文件 {filename} 时出错: {e}"); continue
        ...
        for image in bpy.data.images:
            if image.source == 'FILE' and image.filepath:
                ...
                except Exception as e: self.report({'WARNING'}, f"更新图片 '{image.name}' 的路径时出错: {e}")
